...
def chunking_by_seperators(
    ...
    splitter = SeparatorSplitter(
        separators=[tiktoken_model.encode(s) for s in PROMPTS["default_text_separator"]],
...
async def extract_hierarchical_entities(
    ...
    hierarchical_clustered_entities_relations = await hierarchical_cluster.perform_clustering(
    ...
    hierarchical_clustered_relations = [
        [x for x in y if "src_id" in x.keys()] for y in hierarchical_clustered_entities_relations
    ]
    ...
    if entity_vdb is not None:
        data_for_vdb = {  # key is the md5 hash of the entity name string
            compute_mdhash_id(dp["entity_name"], prefix="ent-"): {
                ...
                + dp["description"],  # entity name and description construct the content
...
async def _pack_single_community_describe(
    ...
    if need_to_use_sub_communities or force_to_use_sub_communities:
        ...
        report_describe, report_size, contain_nodes, contain_edges = (
            _pack_single_community_by_sub_communities(community, max_token_size, already_reports)
        )
...
async def _find_most_related_community_from_entities(
    ...
    use_community_reports = (
        truncate_list_by_token_size(  # in case community reprot is longer than token limitation
...
async def _build_hierarchical_query_context(
    ...
    try:
        path = find_path_with_required_nodes(
            knowledge_graph_inst._graph, key_entities[0], key_entities[-1], key_entities[1:-1]
        )
        # path = list(set(path))
    ...
    reasoning_path_section_list = [["id", "source", "target", "description", "weight", "rank"]]
...
async def _build_higlobal_query_context(
    ...
    logger.info(f"Using {len(use_communities)} communities, {len(use_text_units)} text units")
...
async def _build_hilocal_query_context(
    ...
    relation_section_list = [["id", "source", "target", "description", "weight", "rank"]]
    for i, e in enumerate(use_relations):
