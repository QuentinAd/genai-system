...
def fit_gaussian_mixture(n_components, embeddings, random_state):
    gm = GaussianMixture(
        n_components=n_components, random_state=random_state, n_init=5, init_params="k-means++"
    )
    gm.fit(embeddings)
...
def GMM_cluster(embeddings: np.ndarray, threshold: float, random_state: int = 0):
    ...
    gm = GaussianMixture(
        n_components=n_clusters, random_state=random_state, n_init=5, init_params="k-means++"
    )
    gm.fit(embeddings)
...
def perform_clustering(
    ...
    for i in tqdm(range(n_global_clusters)):
        ...
        # embedding indices
        indices = [embedding_to_index[tuple(embedding)] for embedding in global_cluster_embeddings_]
...
class Hierarchical_Clustering(ClusteringAlgorithm):
    async def perform_clustering(
        ...
        for layer in range(layers):
            ...
            cluster_sparsity = 1 - sum([x * (x - 1) for x in cluster_sizes.values()]) / (
                len(nodes) * (len(nodes) - 1)
            )
            cluster_sparsity_change_rate = abs(cluster_sparsity - pre_cluster_sparsity) / (
