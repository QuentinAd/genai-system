...
async def GLM_embedding(texts: list[str]) -> np.ndarray:
    ...
    client = AsyncOpenAI(api_key=GLM_API_KEY, base_url=GLM_URL)
    ...
    for datapoint in dataset:
        if dataset_name == "musique":
            ...
            queries.append(
                Query(
                    question=datapoint["question"].encode("utf-8").decode(),
                    answer=datapoint["reference"][
                        -1
                    ],  # nq_rear uses "reference" instead of "answer"
                    evidence=supporting_facts,
                )
            )
        elif dataset_name == "popqa":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Report scores after benchmarking.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="top-m entities selection in each community after retrieval.",
    )
    parser.add_argument(
        "--mode", default="hi", help="HiRAG query mode."
    )  # 'no' mode is for simple LLM answering
    ...
    if args.benchmark:
        ...
        async def _query_task_hi(query: Query, mode: str) -> Dict[str, Any]:
            if mode != "no":
                answer = await grag.aquery(
                    query.question,
                    QueryParam(
                        mode=mode, top_k=args.topk, top_m=args.topm, only_need_context=False
                    ),
                )
            ...
            logging.info(
                f"ground truth: {query.answer.lower()}, generated: {answer_short.lower().strip().strip('.')}\n"
            )
            return {
        ...
        async def _run_hi(mode: str):
            answers = [
                await a
                for a in tqdm(
                    asyncio.as_completed([_query_task_hi(query, mode=mode) for query in queries]),
                    total=len(queries),
                )
            ]
            return answers
        ...
        with open(
            f"./datasets/{args.dataset}/{args.dataset}_{args.n}_{args.mode}_hi.json", "w"
        ) as f:
            json.dump(answers, f, indent=4)

    if args.benchmark or args.score:
        with open(
            f"./datasets/{args.dataset}/{args.dataset}_{args.n}_{args.mode}_hi.json", "r"
        ) as f:
            answers = json.load(f)
