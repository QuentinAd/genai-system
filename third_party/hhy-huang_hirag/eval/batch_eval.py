    ...
    with open(query_file, "r", encoding="utf-8") as infile:
        for line_number, line in enumerate(infile, start=1):
            ...
            try:
                ...
                print(f"JSON decoding error in file {query_file} at line {line_number}: {e}")
...
def eval_oq_glm(query_file, result1_file, result2_file, output_file_path):
    ...
    with jsonlines.open(
        output_file_path.replace(".jsonl", "_result_glm.jsonl"), mode="w"
    ) as writer:
...
def eval_oq_deepseek(query_file, result1_file, result2_file, output_file_path):
    ...
    with jsonlines.open(
        output_file_path.replace(".jsonl", "_result_deepseek.jsonl"), mode="w"
    ) as writer:
        for eval_item in evaluations:
            writer.write(eval_item)
...
def eval_oq_openai(query_file, result1_file, result2_file, output_file_path):
    ...
    with jsonlines.open(
        output_file_path.replace(".jsonl", "_result_openai.jsonl"), mode="w"
    ) as writer:
        for eval_item in evaluations:
            writer.write(eval_item)
...
def fetch_eval_result_openai_batch(batch_id, output_file):
    ...
    result_0 = json.loads(
        "\n".join(
            result[0]["response"]["body"]["choices"][0]["message"]["content"]
            .strip()
            .split("\n")[1:-1]
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-q", "--query_file", type=str, default=f"./datasets/{DATASET}/{DATASET}.jsonl"
    )
    parser.add_argument(
        "-r1",
        "--result1_file",
        type=str,
        default=f"./datasets/{DATASET}/{DATASET}_kag_result_deepseek.jsonl",
    )
    parser.add_argument(
        "-r2",
        "--result2_file",
        type=str,
        default=f"./datasets/{DATASET}/{DATASET}_hi_bridge_result_deepseek_pro.jsonl",
    )
    parser.add_argument(
        "-o",
        "--output_file",
        type=str,
        default=f"./datasets/{DATASET}/{DATASET}_eval_hi_graphtrag.jsonl",
    )
    parser.add_argument("-m", "--mode", type=str, default="result", help="request or result")
    parser.add_argument(
        "-api", "--api", type=str, default="openai", help="openai or deepseek or glm"
    )
